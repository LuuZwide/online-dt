...
def vec_evaluate_episode_rtg(
    ...
    for t in range(max_ep_len):
        ...
        if mode != "delayed":
            pred_return = target_return[:, -1] - (reward * reward_scale) # type: ignore
        else:
            pred_return = target_return[:, -1] # type: ignore
        target_return = torch.cat(
            [target_return, pred_return.reshape(num_envs, -1, 1)], dim=1 # type: ignore
        )
